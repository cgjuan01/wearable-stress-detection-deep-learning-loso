...
Download WESAD (link below), unzip so that `WESAD/S2/S2.pkl` exists, then run in this order. Steps 1 and 2 need a GPU for the CNN (about 20 minutes each on a T4; pass `--no-cnn` to skip it). Steps 3 and 4 read `features.csv` and run in seconds on CPU. Step 5 needs a GPU.

```
pip install -r requirements.txt

# 1, 2. features, GBM and CNN, two seeds
#       -> features.csv, loso_per_subject_seed{0,1}.csv, loso_summary_seed{0,1}.csv
python train_loso.py --root /path/to/WESAD --out . --seed 0
python train_loso.py --root /path/to/WESAD --out . --seed 1

# 3. ablation, within-subject normalisation, stress vs amusement
#       -> ablation.csv, loso_per_subject_subjnorm.csv, stress_vs_amusement.csv
python ablation.py --features features.csv --out .

# 4. unsupervised detection in feature space
#       -> anomaly_features_mahalanobis.csv
python anomaly_features.py --features features.csv --out .

# 5. unsupervised detection on raw signal
#       -> anomaly_per_subject_seed0.csv
python anomaly.py --root /path/to/WESAD --out . --seed 0

# smoke test without the dataset (synthetic data; expect perfect scores, they mean nothing)
python make_fake_wesad.py /tmp/fake && python train_loso.py --root /tmp/fake --out /tmp/res --no-cnn
```

Every number in the tables above is read from the committed CSV files. Seeds are fixed; the CNN and autoencoder can differ at the third decimal across hardware.

Data: WESAD, https://ubi29.informatik.uni-siegen.de/usi/data_wesad.html (2.1 GB; not redistributed here). Per-subject metric files are committed at the repo root; `features.csv` is derived from the dataset and is not.
