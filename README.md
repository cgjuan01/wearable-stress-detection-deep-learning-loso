...
python src/train_loso.py       --root /path/to/WESAD --out results --seed 0    # features + GBM + CNN
python src/train_loso.py       --root /path/to/WESAD --out results --seed 1
python src/ablation.py         --features results/features.csv --out results   # ablation, normalisation, stress vs amusement
python src/anomaly_features.py --features results/features.csv --out results   # Mahalanobis, baseline-only
python src/anomaly.py          --root /path/to/WESAD --out results             # autoencoder, baseline-only
python src/make_fake_wesad.py /tmp/fake && python src/train_loso.py --root /tmp/fake --no-cnn   # smoke test, no data needed
```
